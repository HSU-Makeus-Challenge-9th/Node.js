...
export const handleStoreAdd = async (req, res, next) => {
    ...
    */
    try {
        ...
        const ownerId = typeof req.user.id === 'bigint' ? req.user.id : BigInt(req.user.id);
        const store = await storeAdd({ ...bodyToStoreAdd(req.body), ownerId: ownerId });
